...
int main(int argc, char *argv[]) {
  ...
  if (save == NULL) {
    fprintf(output, "<error>Niemoge przeczytac pliku</error>");
    return 0;
  }
  
  if (!D2StestMagick(save)) {
    fprintf(output, "<error>Nieprawidlowe klucze kontrolne</error>");
    D2Sfree(save);
    return 0;
  }
  ...
  fprintf(output, "<character");
    ...
    fprintf(output, " type=\"%d\"", save->header->characterType);
