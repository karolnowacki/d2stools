...
typedef struct {
	...
	char characterName[17]; //orginal 16!!

	union {
	  unsigned char value;
	  struct {
	    unsigned int newcharacter : 1;
	    unsigned int unk1 : 1;
	    unsigned int hardcore : 1;
	    unsigned int hasdied : 1;
	    unsigned int unk2 : 1;
	    unsigned int expansion : 1;
	    unsigned int lader : 1;
	    unsigned int unk3 : 1;
	  } flags;
	} characterType;
...
typedef struct {
	...
	u16 questStatus[6];
	u16 end;
} __attribute__ ((packed)) D2S_actinfo ;

typedef struct {
	u32 magick; //Woo! 0x216f6f57
	u32 acts;
	u16 size;
} __attribute__ ((packed)) D2S_questinfoheader;

typedef struct {
	u16 magick; // WS 0x5357
	u8 unknown[6];
	u32 wp[3 * 6];
} __attribute__ ((packed)) D2S_waypoints;
...
typedef struct {
	...
	u32 nightmare1[2];
	u32 hell1[2];
} __attribute__ ((packed)) D2S_npcstate;

typedef struct {
	u16 magick; //if 0x6669
	u8 skill[30];
} __attribute__ ((packed)) D2S_skills;
